...
import { DeepSeekAuth } from "./deepseek-auth"
import { createAuthentication } from "@/app/bootstrap/create-authentication"

async function fixture(run: (auth: DeepSeekAuth, store: FileAuthStore) => Promise<void>) {
    const directory = await mkdtemp(join(tmpdir(), "buli-deepseek-auth-"))
    const store = new FileAuthStore(join(directory, "auth.json"))
    const auth = new DeepSeekAuth({ store })
...
test("stores a local API key without changing OpenAI and removes only DeepSeek", async () => {
    await fixture(async (auth, store) => {
        ...
        const kimi = { type: "api_key", key: "synthetic-kimi" } as const
        await store.set("kimi-coding", kimi)
        ...
        expect(await store.get("deepseek")).toEqual({ type: "api_key", key: "synthetic-key" })
        expect(await auth.status()).toEqual({ providerId: "deepseek", connected: true })
        ...
        expect(await store.get("kimi-coding")).toEqual(kimi)
    })
})
...
test("rejects invalid methods, key format and stored credential types without leaking keys", async () => {
    await fixture(async (auth, store) => {
        ...
        for (const key of ["", " ", "secret with spaces", "secret\u0000suffix"]) {
            await expect(auth.login("api-key", interaction(async () => key))).rejects.toThrow("Invalid DeepSeek API key format")
        }
...
test("root cancellation aborts a prompt and waits for its cleanup", async () => {
    await fixture(async (_auth, store) => {
        ...
        const auth = new DeepSeekAuth({ store, signal: root.signal })
...
test("transport rejects missing credentials and work after disposal without fetch", async () => {
    await fixture(async (_auth, store) => {
        let calls = 0
        const auth = new DeepSeekAuth({ store, fetch: Object.assign(async () => {
            calls++
            return new Response(null, { status: 204 })
        }, { preconnect: globalThis.fetch.preconnect }) })
        await expect(auth.authenticatedFetch("https://api.deepseek.com/models"))
            .rejects.toThrow("check local credentials")
        await auth.dispose()
        await expect(auth.authenticatedFetch("https://api.deepseek.com/models")).rejects.toThrow()
        expect(calls).toBe(0)
    })
})
...
for (const mode of ["caller", "dispose"] as const) {
    test(`${mode} aborts an unread response body and awaits its cancellation`, async () => {
        await fixture(async (_auth, store) => {
            await store.set("deepseek", { type: "api_key", key: "synthetic-key" })
            const cancelled = Promise.withResolvers<void>()
            const cleanup = Promise.withResolvers<void>()
            const controller = new AbortController()
            let fetchSignal: AbortSignal | null | undefined
            const auth = new DeepSeekAuth({ store, fetch: Object.assign(async (_input: RequestInfo | URL, init?: RequestInit) => {
            ...
            try {
                const response = await auth.authenticatedFetch("https://api.deepseek.com/models", { signal: controller.signal })
                ...
                await expect(response.text()).rejects.toThrow("DeepSeek response was cancelled")
                cleanup.resolve()
                await disposing
...
test("dispose waits for a pending credential read and prevents a late request", async () => {
    await fixture(async (_auth, store) => {
        ...
        const result = auth.authenticatedFetch("https://api.deepseek.com/models").catch((error: unknown) => error)
        await entered.promise
        const disposing = auth.dispose()
        let disposed = false
        void disposing.then(() => { disposed = true })
...
test("bootstrap registers all providers using the injected store", async () => {
    await fixture(async (_auth, store) => {
        ...
        try {
            ...
            await composition.service.login("deepseek", "api-key", interaction(async () => "local-only"))
            expect(await store.get("deepseek")).toEqual({ type: "api_key", key: "local-only" })
        } finally {
