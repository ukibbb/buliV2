...
import type { IBuliPathSuggestion, TBuliPathSearcher } from "@/app/contracts"
...
async function suggestionsFromCandidates(
    ...
    const suggestions: IBuliPathSuggestion[] = []
    const seen = new Set<string>()
    for (const candidate of candidates) {
        signal.throwIfAborted()
        let path: string
        let kind: IBuliPathSuggestion["kind"]
        try {
            path = await realpath(candidate)
...
async function directSuggestion(
    workspace: string,
    query: string,
): Promise<IBuliPathSuggestion | undefined> {
    const expanded = expandHome(query)
...
function suggestionScore(
    suggestion: IBuliPathSuggestion,
    needle: string,
): number {
