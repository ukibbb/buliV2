export interface IKimiFetchOptions {
...
export function createKimiFetch(options: IKimiFetchOptions): typeof fetch {
    const run = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        try {
            ...
            throw new Error("Invalid Kimi request")
        }
        ...
        if (!isModels && !isChat) {
            throw new Error("Kimi request endpoint or method is not allowed")
        }
        ...
        try {
            ...
            signal.throwIfAborted()
            throw new Error("Kimi authenticated request failed; check local credentials and connectivity")
        }
    }
...
async function readJsonObjectBody(request: Request, signal: AbortSignal): Promise<string> {
    if (!request.body) throw new Error("Invalid Kimi JSON request body")
    ...
    try {
        ...
        cancel()
        signal.throwIfAborted()
        throw new Error("Invalid Kimi JSON request body")
    } finally {
...
function protectResponse(
    ...
    const abort = () => {
        if (finished) return
        controller.error(new Error("Kimi response was cancelled"))
        void cancel()
    }
    const body = new ReadableStream<Uint8Array>({
        ...
        async pull() {
            try {
                ...
                if (finished || signal.aborted) return
                controller.error(new Error("Kimi response stream failed"))
                await cancel()
            }
        },
        cancel,
