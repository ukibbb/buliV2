...
import { createKimiFetch } from "@/providers/kimi/transport/kimi-fetch"

export interface IKimiAuthOptions {
    readonly store: IAuthStore
    readonly signal?: AbortSignal
    readonly fetch?: typeof fetch
}

export class KimiAuth implements IAuthenticationProvider {
    readonly id = "kimi-coding"
    readonly name = "Kimi Code (.com)"
    readonly methods = [{
        id: "api-key",
        name: "API key",
        description: "Store a Kimi Code .com API key locally (not verified online)",
    }] as const
    ...
    constructor(private readonly options: IKimiAuthOptions) {
        const transport = createKimiFetch({
            ...
            requireApiKey: async (signal) => {
                ...
                if (!credential || credential.type !== "api_key") {
                    throw new Error("Kimi Code requires a stored API key")
                }
    ...
    readonly status = (signal?: AbortSignal): Promise<IAuthStatus> => this.track(async () => {
        ...
        if (credential) {
            if (credential.type !== "api_key") throw new Error("Kimi Code requires an API key")
    ...
    readonly login = (methodId: string, interaction: IAuthInteraction): Promise<IAuthStatus> => this.track(async () => {
        ...
        if (methodId !== "api-key") throw new Error("Unsupported Kimi Code authentication method")
        this.activeLogin?.abort(new Error("Kimi Code login was replaced"))
        ...
        try {
            ...
            const input = await interaction.prompt({
                type: "secret",
                message: "Enter your Kimi Code .com API key (stored locally; not verified online):",
                placeholder: "API key",
                signal,
            })
            signal.throwIfAborted()
            ...
            signal.throwIfAborted()
            if (!committed) throw new Error("Kimi Code login was replaced")
    ...
    readonly logout = (signal: AbortSignal): Promise<boolean> => this.track(async () => {
        const operationSignal = this.signal(signal)
        this.activeLogin?.abort(new Error("Kimi Code login was cancelled by logout"))
        this.activeLogin = undefined
        return this.options.store.remove(this.id, operationSignal)
    })

    readonly dispose = (reason: unknown = new Error("Kimi Code authentication is shutting down")): Promise<void> => {
...
function validateApiKey(input: string): string {
    ...
    if (!key || /[\s\u0000-\u001f\u007f]/u.test(key)) {
        throw new Error("Invalid Kimi Code API key format")
    }
    return key
}
