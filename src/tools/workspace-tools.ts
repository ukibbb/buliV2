...
import {
  BuliToolRegistry,
  type IBuliToolDefinition,
} from "@/tools/tool-registry"

const MAX_TOOL_OUTPUT_CHARACTERS = 100_000

const readFile: IBuliToolDefinition = {
  name: "read_file",
  ...
  execute: async (input, context) => {
    ...
    const file = await realpath(resolve(context.workspaceRoot, path))

    if (!file.startsWith(`${context.workspaceRoot}${sep}`)) {
...
const glob: IBuliToolDefinition = {
  name: "glob",
...
const grep: IBuliToolDefinition = {
  name: "grep",
...
export function createWorkspaceToolRegistry(): BuliToolRegistry {
  return new BuliToolRegistry([readFile, glob, grep])
}
