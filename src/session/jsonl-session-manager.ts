...
export class JsonlSessionManager implements ISessionManager {
    ...
    // ?? what appendMessages does one by line
    // `appendMessage` przyjmuje jedną zakończoną wiadomość i najpierw sprawdza,
    // czy można ją trwale zapisać, np. odrzuca odpowiedź asystenta ze stanem `pending`.
    // `JSON.stringify` zamienia wiadomość na jeden rekord JSON, a `\n` kończy jego
    // wiersz. Znaki nowej linii wewnątrz treści są escapowane, więc format JSONL
    // nadal ma dokładnie jeden rekord na wiersz. `appendFileSync` dopisuje rekord
    // bez nadpisywania historii, a potem ten sam komunikat trafia do pamięci.
