...
import { createDeepSeekFetch } from "./deepseek-fetch"
...
for (const [url, method] of [
    ...
    ["http://api.deepseek.com/models", "GET"],
    ...
    ["http://api.deepseek.com/chat/completions", "POST"],
    ["https://api.deepseek.com/messages", "POST"],
]) {
...
for (const body of [undefined, "", "{", "null", "[]", "1", '"synthetic-secret"']) {
    test(`rejects invalid chat JSON before credentials: ${String(body)}`, async () => {
        ...
        await expect(transport(CHAT_URL, { method: "POST", ...(body === undefined ? {} : { body }) }))
            .rejects.toThrow("Invalid DeepSeek JSON request body")
        expect(touched).toBe(false)
    })
}
...
test(`rejects redirects even when an injected fetch ignores redirect policy: ${endpoint}`, async () => {
    ...
    await expect(transport(endpoint, endpoint === CHAT_URL ? { method: "POST", body: "{}" } : {})).rejects.toThrow("DeepSeek authenticated request failed")
    expect(cancelled).toBe(true)
})
}
...
test("body lifecycle handles EOF, read failures and consumer cancellation", async () => {
    for (const mode of ["eof", "error", "cancel"] as const) {
        ...
        if (mode === "error") await expect(response.text()).rejects.toThrow("DeepSeek response stream failed")
